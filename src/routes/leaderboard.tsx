...
import { familyLeaderboard, globalLeaderboard, type LeaderboardEntry } from "@/data/mock";
...
function LeaderboardPage() {
  ...
  const list = tab === "family" ? familyLeaderboard : globalLeaderboard;
  const max = Math.max(...list.map((e) => e.xp));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {list.map((e) => <Row key={e.rank} entry={e} max={max} />)}
      </div>
    </div>
  );
}
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        {entry.rank === 1 ? <Crown className="size-6" /> : entry.rank <= 3 ? <Medal className="size-5" /> : entry.rank}
      </div>
      <div className="size-12 shrink-0 rounded-2xl grid place-items-center text-base font-extrabold text-white shadow-pop-sm" style={{ backgroundColor: entry.avatarColor }}>
        {entry.name.slice(0, 1)}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="font-display font-extrabold truncate">{entry.name}</span>
          <span className="text-[10px] font-extrabold uppercase tracking-wide px-1.5 py-0.5 rounded bg-warning/20 text-warning">LVL {entry.level}</span>
        </div>
        <div className="h-2 rounded-full bg-muted overflow-hidden">
          <div className="h-full bg-gradient-to-r from-primary to-primary-dark rounded-full" style={{ width: `${pct}%` }} />
        </div>
      </div>
      <div className="font-display font-extrabold text-lg tabular-nums shrink-0">{entry.xp.toLocaleString()}</div>
    </div>
  );
}
